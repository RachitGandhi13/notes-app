          ...
          <div className="mt-6">
            {purchased ? (
              <Link
                ...
                className="bg-primary text-primary-foreground inline-flex items-center gap-2 rounded-lg px-6 py-3 font-semibold transition-opacity hover:opacity-90"
              >
        ...
        <ul className="divide-y rounded-xl border">
          {allContent.map((item, i) => (
            <li key={item.id} className="flex items-center gap-3 px-4 py-3">
              <span className="text-muted-foreground w-6 shrink-0 text-sm">{i + 1}</span>
              <PlayCircle className="text-muted-foreground h-4 w-4 shrink-0" />
              <span className="flex-1 text-sm">{item.title}</span>
              {item.folderTitle && (
                <span className="text-muted-foreground text-xs">{item.folderTitle}</span>
              )}
            </li>
          ))}
        </ul>
